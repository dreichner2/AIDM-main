...
def create_app() -> Flask:
    ...
    def _apply_api_guards():
        ...
        telemetry_metric('api.requests_total', 1, tags={'path': request.path, 'method': request.method})

        if request.path == '/api/health':
            return None

        if app.config.get('AIDM_AUTH_REQUIRED') and not request_is_authorized():
            telemetry_event(
                'api.unauthorized',
                payload={'path': request.path, 'method': request.method, 'remote_addr': request.remote_addr},
                severity='warning',
            )
